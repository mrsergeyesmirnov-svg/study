...
| Адрес шоурума | Шоурум СПб — «Примерить сегодня» |
...
### Этап 2.5 — Планировщик постов + импорт из канала ✅

**Два потока (гибрид):**

1. **Админка → планировщик (предпочтительно)**  
   Создать товар с биркой → «Отложить на время» → в `publishAt` уходит в канал + каталог (тик каждые 30 с).

2. **Ручной пост в канал → входящие → скан бирки**  
   Бот читает `channel_post`. Если текст похож на карточку (название + цена, опц. замеры/размер/состояние) — создаётся **черновик без бирки** (не в каталоге).  
   Админ: **Входящие из канала** → «Привязать бирку» (скан) → товар **AVAILABLE** в каталоге, в пост дописывается ссылка «Оформить заказ».

**Почему не «сразу из канала в каталог»:** без бирки нельзя продать офлайн сканом и нет URL `/i/VTG-…`. Скан — явный шаг «вещь на полке = в системе».

**Форма поста для импорта (примеры):**
```
Футболка Oakley 00s
...
Свои автопосты с `VTG-…` / `/i/VTG-` повторно не импортируются.

**Результат:** «завтра в 17:00» из админки; ручные посты в TG не теряются — дожидаются бирки во входящих.
